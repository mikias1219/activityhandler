...
urlpatterns = [
    ...
    path(
        "habits/<int:habit_pk>/check-ins/",
        HabitCheckInListCreateView.as_view(),
        name="habit-check-ins",
    ),
    path(
        "habits/<int:habit_pk>/check-ins/<int:pk>/",
        HabitCheckInDetailView.as_view(),
        name="habit-check-in-detail",
    ),
